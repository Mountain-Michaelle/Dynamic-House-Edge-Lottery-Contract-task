...
contract EthexLoto is Ownable {
    ...
    function placeBet(bytes22 params) external payable {
        ...
        require(markedCount > 0, "At least one cell should be marked");
        houseEdge = getHouseEdge(markedCount);
        uint256 jackpotFee = msg.value * JACKPOT_PERCENT * PRECISION / 100 / PRECISION;
        uint256 houseEdgeFee = msg.value * houseEdge * PRECISION / 100 / PRECISION;
        uint256 betAmount = msg.value - jackpotFee - houseEdgeFee;
        
        (coefficient, markedCount, holdAmount) = getHold(betAmount, bet);
        
        require(msg.value * (100 - JACKPOT_PERCENT - houseEdge) * (coefficient * 8 - 15 * markedCount) <= 9000 ether * markedCount);
        
        require(
            msg.value * (800 * coefficient - (JACKPOT_PERCENT + houseEdge) * (coefficient * 8 + 15 * markedCount)) <= 1500 * markedCount * (address(this).balance - holdBalance));
    ...
    function getHouseEdge(uint8 markedCount) internal pure returns (uint256 houseEdge) {
        if (markedCount == 1)
            return 12;
        if (markedCount < 4)
            return 10;
